...
public class CountWorkingDays {
    public static void main(String[] args) throws IOException, ParseException {
        ...
        startDate.setTime(format.parse(reader.readLine()));
        endDate.setTime(format.parse(reader.readLine()));
        ...
        for (Calendar currentDate = startDate; currentDate.before(endDate); currentDate.add(Calendar.DATE,1)){
            if (currentDate.get(Calendar.DAY_OF_WEEK) != Calendar.SATURDAY
                ...
                for (Calendar holiday : holidays){
                    if (currentDate.get(Calendar.DAY_OF_MONTH) == holiday.get(Calendar.DAY_OF_MONTH)
                            && currentDate.get(Calendar.MONTH) == holiday.get(Calendar.MONTH)){
                        isNotHoliday = false;
                        break;
                    }
                }
